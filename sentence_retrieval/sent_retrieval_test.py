...
# %%
model_path = '/nfs/nas-6.1/wclu/AICUP/2023/NCKU-AICUP2023-baseline/sent/checkpoints/lert-large-doc12-xnli_e1_20e/checkpoint-700'
from transformers import pipeline, AutoTokenizer, AutoModelForSequenceClassification
model = AutoModelForSequenceClassification.from_pretrained(model_path, cache_dir = '/nfs/nas-6.1/wclu/cache', num_labels=2)
...
# %%
'''
test_data = load_json(f"../data/test_doc{NUM_DOC}.jsonl")
test_df = pair_with_wiki_sentences_eval(
    mapping,
...
test_df['target'] = [0 for i in range(len(test_df))]
test_dataset = CustomDataset(test_df, tokenizer, 512, 512)
result = trainer.predict(test_dataset)
probs = [x.softmax(dim=0)[1] for x in torch.tensor(result.predictions)]
evaluate_retrieval(
    probs=probs,
    df_evidences=test_df,
    ground_truths=test_data,
    top_n=5,
    cal_scores=False,
    save_name=f"public_test_doc{NUM_DOC}sent5_{model_name}.jsonl",
)
'''

test_data = load_json(f"../data/test_private_doc{NUM_DOC}.jsonl")
test_df = pair_with_wiki_sentences_eval(
    mapping,
    pd.DataFrame(test_data),
    is_testset=True,
...
evaluate_retrieval(
    ...
    top_n=5,
    cal_scores=False,
    save_name=f"private_test_doc{NUM_DOC}sent5_{model_name}.jsonl",
)

'''
# %%
train_df = pair_with_wiki_sentences_eval(
    mapping=mapping,
    df=pd.DataFrame(TRAIN_GT),
)
train_df['target'] = [0 for i in range(len(train_df))]
...
evaluate_retrieval(
    probs=probs,
    df_evidences=train_df,
    ground_truths=TRAIN_GT,
    top_n=5,
    cal_scores=False,
    save_name=f"train_doc{NUM_DOC}sent5_{model_name}.jsonl",
)
...
result = trainer.predict(dev_dataset)
probs = [x.softmax(dim=0)[1] for x in torch.tensor(result.predictions)]
...
evaluate_retrieval(
    ...
    top_n=5,
    cal_scores=False,
    save_name=f"dev_doc{NUM_DOC}sent5_{model_name}.jsonl",
)
